...
namespace Common
{
	...
	void ConditionVariable::TimedWait(const int& msWait) 
	{
		Event* waiting_event;
		HANDLE handle;
		{
		...
		{
			AutoUnlock unlock(user_lock_);  // Release caller's lock
			WaitForSingleObject(handle, static_cast<DWORD>(msWait));
			// Minimize spurious signal creation window by recycling asap.
			AutoLock auto_lock(internal_lock_);
			RecycleEvent(waiting_event);
			// Release internal_lock_
    ...
    bool ConditionVariable::WaitForTimeout(const Time& absTime)
    {
		Event* waiting_event;
		HANDLE handle;

        Time now = Time::Now();
        if (now > absTime) return true;
        TimeDelta td = absTime - now;
        DWORD msWait = static_cast<DWORD>(td.InMilliseconds());
        DWORD dwEvent = 0xFFFFFFFF;
		{
			AutoLock auto_lock(internal_lock_);
			if (RUNNING != run_state_) return true;  // Destruction in progress.
			waiting_event = GetEventForWaiting();
			handle = waiting_event->handle();
			assert(handle);
		}  // Release internal_lock.
		...
		{
			AutoUnlock unlock(user_lock_);  // Release caller's lock
			dwEvent = WaitForSingleObject(handle, msWait);
			// Minimize spurious signal creation window by recycling asap.
			AutoLock auto_lock(internal_lock_);
			RecycleEvent(waiting_event);
			// Release internal_lock_
		}  // Reacquire callers lock to depth at entry.
        assert(dwEvent == WAIT_OBJECT_0 || dwEvent == WAIT_TIMEOUT );
        return dwEvent == WAIT_TIMEOUT;
    }
...
#define _ConditionVariableImplemented
...
namespace Common
{
	...
	void ConditionVariable::TimedWait(const int& msWait) 
	{
		...
		static const int64_t nMilliSecondsPerSecond = 1000;
		static const int64_t nNanoSecondsPerMilliSecond = 1000;
		static const int64_t nNanoSecondsPerSecond = 1000000000;


		struct timespec abstime;
		abstime.tv_sec = now.tv_sec + (msWait / nMilliSecondsPerSecond);
		abstime.tv_nsec = (now.tv_usec + (msWait % nMilliSecondsPerSecond)) * nNanoSecondsPerMilliSecond;
		abstime.tv_sec += abstime.tv_nsec / nNanoSecondsPerSecond;
		abstime.tv_nsec %= nNanoSecondsPerSecond;
		assert(abstime.tv_sec >= now.tv_sec);  // Overflow paranoia
	...
	bool ConditionVariable::WaitForTimeout(const Time& absTime) 
	{
            struct timespec tstime = absTime.ToTimeSpec();
#if !defined(NDEBUG)
		user_lock_->CheckHeldAndUnmark();
#endif
		int rv = pthread_cond_timedwait(&condition_, user_mutex_, &tstime);
		assert(rv == 0 || rv == ETIMEDOUT);
#if !defined(NDEBUG)
		user_lock_->CheckUnheldAndMark();
#endif
...
// check that we got an implementation
#ifndef _ConditionVariableImplemented
#error "No implementation for ConditionVariable"
#endif // end #ifndef _ConditionVariableImplemented