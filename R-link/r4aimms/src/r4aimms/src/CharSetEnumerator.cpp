...
namespace{
    static const char* g_szIconvCharSetNames[] = {
    ...
    size_t g_nIconvCharSetNames = sizeof(g_szIconvCharSetNames)/sizeof(g_szIconvCharSetNames[0]);
}

namespace Common{
    namespace {

        std::wstring towcs( const char* szSrc){
            std::wstring strDest;
            strDest.resize(strlen(szSrc));
            size_t converted = mbstowcs( &strDest[0], szSrc, strDest.size() );
            if (converted != strDest.size()) {
                throw CharSetConverter::SystemException(errno);
            }
            return strDest;
        }
        bool validCharSet( const char* szCharsetName ){
            CharSetConverter csc;
            return csc.open(szCharsetName,szCharsetName);
        }
    }

    // enumerate the supported or installed CharSets
    void enumerateCharSets( ICharSetEnumeratorCallbackW* cb, EnumeratorFlags enumFlags)
    {
        for (size_t i = 0; i < g_nIconvCharSetNames; ++i){
            if (enumFlags == Supported || validCharSet( g_szIconvCharSetNames[i] ) ){
                cb->onCharSet(towcs(g_szIconvCharSetNames[i]).c_str());
            }
        }
    }

    void enumerateCharSets( ICharSetEnumeratorCallbackA* cb, EnumeratorFlags enumFlags)
    {
        for (size_t i = 0; i < g_nIconvCharSetNames; ++i){
            if (enumFlags == Supported || validCharSet( g_szIconvCharSetNames[i] ) ){
                cb->onCharSet(g_szIconvCharSetNames[i]);
            }
        }
    }
}
