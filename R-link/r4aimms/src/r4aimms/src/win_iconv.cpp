...
int kernel_mbtowc_inline(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize);
static int kernel_mbtowc(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize);
int kernel_wctomb_inline(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize);
...
int utf16_mbtowc_inline(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize);
static int utf16_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize);
int utf16_wctomb_inline(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize);
...
{
    rec_iconv_t *cd = (rec_iconv_t *)_cd;
    size_t r = cd->iconv(cd->cd, (const char**)inbuf, inbytesleft, outbuf, outbytesleft);
    errno = *(cd->_errno());
    return r;
}
...
win_iconv(iconv_t _cd, const char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft)
{
    ...
    while (*inbytesleft != 0)
    {
		...
		if (cd->from.mbtowc == kernel_mbtowc) {
			insize = kernel_mbtowc_inline(&cd->from, (const uchar *)*inbuf, (int)*inbytesleft, wbuf, &wsize);
		} else if (cd->from.mbtowc == utf16_mbtowc) {
			insize = utf16_mbtowc_inline(&cd->from, (const uchar *)*inbuf, (int)*inbytesleft, wbuf, &wsize);
		} else {
		...
		if (cd->to.wctomb == utf16_wctomb) {
			outsize = utf16_wctomb_inline(&cd->to, wbuf, wsize, (uchar *)*outbuf, (int)(*outbytesleft));
		} else if (cd->to.wctomb == kernel_wctomb) {
			outsize = kernel_wctomb_inline(&cd->to, wbuf, wsize, (uchar *)*outbuf, (int)(*outbytesleft));
		} else {
...
__forceinline int kernel_mbtowc_inline(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize)
{
    int len;
    len = (cv->mblen == sbcs_mblen) ? 1 : cv->mblen(cv, buf, bufsize);
	if (len==1 && *buf < 0x80) {
		*wbuf = *buf;
		*wbufsize = 1;
		return 1;
...
static int kernel_mbtowc(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize)
{
	return kernel_mbtowc_inline(cv, buf, bufsize, wbuf, wbufsize);
}


__forceinline int kernel_wctomb_inline(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize)
{
...
static int kernel_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize)
{
	return kernel_wctomb_inline(cv, wbuf, wbufsize, buf, bufsize);
}
/*
...
__forceinline int utf16_mbtowc_inline(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize)
{
...
static int utf16_mbtowc(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize)
{
	return utf16_mbtowc_inline(cv, buf, bufsize, wbuf, wbufsize);
}

__forceinline int utf16_wctomb_inline(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize)
{
    if ((cv->flags & FLAG_USE_BOM) && !(cv->mode & UNICODE_MODE_BOM_DONE))
    {
        int r;
...
static int utf16_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize)
{
	return utf16_wctomb_inline(cv, wbuf, wbufsize, buf, bufsize);
}
