#ifndef _INCLUDED__Common_Lock
#define _INCLUDED__Common_Lock
...
namespace Common
{
	...
	// A helper class that releases the given Lock while the InvertedAutoLock is in scope.
	class InvertedAutoLock {
	public:
		explicit InvertedAutoLock(Lock& lock) : lock_(lock) {
			lock_.AssertAcquired();
			lock_.Release();
		}
	...
	private:
		Lock& lock_;
		DISALLOW_COPY_AND_ASSIGN(InvertedAutoLock);
	};
...
#endif // end #ifndef _INCLUDED__Common_Lock
